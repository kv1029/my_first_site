...
def detect_scam():
    client_ip = request.remote_addr
    logger.info(f"Client IP (scam route): {client_ip}")
    ...
    if file.filename.endswith('.pdf'):
        ...
        return render_template("index.html", message="Invalid file type. Please upload a PDF or TXT file.", client_ip=client_ip)

    if not extracted_text.strip():
        return render_template("index.html", message="File is empty or text could not be extracted.", client_ip=client_ip)
...
def predict_url():
    client_ip = request.remote_addr
    logger.info(f"Client IP (predict route): {client_ip}")
